...
const createSubscriptionSchema = z.object({
    body: z.object({
        name: z.string("Name is required").max(200, "Name can be at most 200 characters"),
        price: z.number().gte(0, 'Price must be a positive') ,
        currency: z.enum(['INR', 'USD', 'EUR', 'JPY'], 'Invalid Currency'),
        frequency: z.enum(['daily', 'weekly', 'monthly', 'quarterly', 'halfYearly', 'yearly'], 'Invalid Frequency'),
        category: z.enum(['sports', 'news', 'entertainment', 'lifestyle', 'technology', 'finance', 'politics', 'other'], 'Invalid Category'),
        paymentMethod: z.string(),
        status: z.enum(['active', 'cancelled', 'expired'], 'Invalid Status for Subscription'),

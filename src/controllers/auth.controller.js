  ...
  try {
    ...
    if (!user) {
      return res.status(404).json({
        error: "User with this Email or Username doesn't exist. Please Sign Up",
      });
    }
  ...
  try {
    ...
    try {
      ...
      if (incomingRefreshToken !== user?.refreshToken) {
        res
          .status(401)
          .json({ error: "Refresh token is either expired or used" });
      }
...
const resetPassword = async (req, res) => {
  try {
    ...
    if (!newPassword) {
      res.status(401).json({ error: "Please send valid Password" });
    }
    const Id = req.user.id; // through auth middleware
    await User.update({
      where: { id: Id },
